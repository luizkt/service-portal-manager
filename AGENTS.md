...
- The SDK code is written for Kotlin.
...
2. Run the tests. This can take a long time so you may prefer to run individual tests.
   ...
   ```bash
   ./gradlew :temporal-sdk:test --offline --tests "com.serviceportal.manager*"
...
## Tests
- All tests for this each package is located in `$PACKAGE_NAME/src/test/kotlin/com/serviceportal/manager`, where `$PACKAGE_NAME` is the name of the package
...
- Code Coverage should be at least 95%.
