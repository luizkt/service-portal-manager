...
class IntegrationControllerTest {
    ...
    private val service: IntegrationService = mockk(relaxUnitFun = true)
    private val controller = IntegrationController(service)

    private val dto = IntegrationDto(
        integrationId = "int-1", version = 1, type = "HTTP",
        url = "http://api/clients", method = "GET", headers = emptyMap(),
        timeout = 5000, bodyTemplate = null, responseBody = null,
        active = true, createdAt = LocalDateTime.now(), updatedAt = LocalDateTime.now()
    )
    private val request = IntegrationRequest(integrationId = "int-1", url = "http://api/clients", method = "GET")
    ...
    @Test fun `GET list status=active returns list`() {
        ...
        assertThat(resp.body as List<IntegrationDto>).hasSize(1)
    }

    @Test fun `GET versions returns list`() {
        every { service.listVersions("int-1", null) } returns listOf(dto)
        val resp = controller.listVersions("int-1", null)
        assertThat(resp.statusCode.value()).isEqualTo(200)
        assertThat(resp.body).hasSize(1)
    }

    @Test fun `GET by id and version returns 200`() {
        every { service.get("int-1", 1) } returns dto
        assertThat(controller.get("int-1", 1).statusCode.value()).isEqualTo(200)
    }

    @Test fun `PUT returns 201 with new version and Location`() {
        val v2dto = dto.copy(version = 2)
        every { service.update("int-1", 1, request) } returns v2dto
        val resp = controller.update("int-1", 1, request)
        assertThat(resp.statusCode.value()).isEqualTo(201)
        assertThat(resp.body?.version).isEqualTo(2)
        assertThat(resp.headers.getFirst("Location")).isEqualTo("/manager/integrations/int-1/versions/2")
    }

    @Test fun `DELETE returns 204`() {
        val resp = controller.delete("int-1", 1)
        assertThat(resp.statusCode.value()).isEqualTo(204)
        verify(exactly = 1) { service.deactivate("int-1", 1) }
    }
}
